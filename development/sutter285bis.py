...
class sutterMP285:
    ...
    def __init__(self):
        self.verbose = True  # level of messages
        self.timeOut = 30  # timeout in sec
        # initialize serial connection to controller
        self.ser = serial.Serial(port='/dev/tty.usbserial', baudrate=9600, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,
        ...
        if currentV == 200:
            ...
            print
            'sutterMP285: WARNING Sutter did not respond at startup.'
    ...
    def gotoPosition(self, pos):
        if len(pos) != 3:
            print
            'Length of position argument has to be three'
            sys.exit(1)
        ...
        self.ser.write(
            'm' + xyzb + '\r')  # send position to controller; add the "m" and the CR to create the move command
        cr = []
        cr = self.ser.read(1)  # read carriage return and ignore
        endt = time.time()  # stop timer
        if len(cr) == 0:
            print
            'Sutter did not finish moving before timeout (%d sec).' % self.timeOut
        else:
            print
            'sutterMP285: Sutter move completed in (%.2f sec)' % (endt - startt)
    ...
    def getStatus(self):
        ...
        if self.verbose:
            print
            'step_mul (usteps/um): %g' % self.stepMult
            print
            'xspeed" [velocity] (usteps/sec): %g' % self.currentVelocity
            print
            'velocity scale factor (usteps/step): %g' % self.vScaleFactor
        #
